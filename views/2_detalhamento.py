...
def renderizar_painel(df_escopo, coluna_agrupamento, label_eixo_x):
    # 1. KPIs
    ...
    fig_tempo = px.bar(
        ...
        barmode='stack',
        color_discrete_map=CORES_STATUS # <-- Aplica as cores definidas na variável
    )
    ...
    if df_concluidos.empty:
        ...
        cor_concluido = CORES_STATUS.get("Concluído", "#2ecc71") # Pega a cor principal para os gráficos de Top 5
...
with aba_diaria:
    ...
    renderizar_painel(df_filtrado, 'Data_Dia', 'Dias')

with aba_mensal:
    st.subheader("Performance Consolidada por Mês")
    renderizar_painel(df_filtrado, 'Ano_Mes', 'Meses')

with aba_anual:
    st.subheader("Performance Histórica Anual")
    renderizar_painel(df_filtrado, 'Ano', 'Anos')